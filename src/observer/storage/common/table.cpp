...
RC Table::create_index(Trx *trx, const char *index_name, int columns_count, const IndexColumn *attribute_names, const int is_unique) {
    if (columns_count==1) {
        char *attribute_name = attribute_names[0].column_name;
        ...
        RC rc = new_index_meta.init(index_name,columns_count,field_meta->name(),is_unique);
        if (rc != RC::SUCCESS) {
            return rc;
        }
        ...
        std::string mutil_attribute_name;
        for (int i=columns_count-1;i>=0;i--){
            char * attribute_name=attribute_names[i].column_name;
            if (i==columns_count-1)
                mutil_attribute_name=attribute_name;
            else
                mutil_attribute_name=mutil_attribute_name+","+attribute_name;
        ...
        printf("%s\n",mutil_attribute_name.c_str());
        if (table_meta_.index(index_name) != nullptr || table_meta_.find_index_by_field(mutil_attribute_name.c_str())){
            return RC::SCHEMA_INDEX_EXIST;
        }
        IndexMeta new_index_meta;
        RC rc = new_index_meta.init(index_name,columns_count,mutil_attribute_name,is_unique);
        TableMeta new_table_meta(table_meta_);
        rc = new_table_meta.add_index(new_index_meta);
        if (rc != RC::SUCCESS) {
            LOG_ERROR("Failed to add index (%s) on table (%s). error=%d:%s", index_name, name(), rc, strrc(rc));
...
RC Table::update_record(Trx *trx, ConditionFilter *filter, const char *attribute_name, const Value *value, int *updated_count) {
    ...
    for ( ; RC::SUCCESS == rc && record_count < INT_MAX; rc = scanner.get_next_record(&record)) {
        if (trx == nullptr || trx->is_visible(this, &record)) {
            rc = update_record(trx,&record,attribute_name,value,updated_count);
            if (rc != RC::SUCCESS) {
                break;
            }
            record_count++;
