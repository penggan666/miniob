...
const static Json::StaticString FIELD_ISNULL("is_null");
...
void FieldMeta::to_json(Json::Value &json_value) const {
  ...
  json_value[FIELD_ISNULL] = is_null_;
}
...
RC FieldMeta::from_json(const Json::Value &json_value, FieldMeta &field) {
  ...
  if (!isnull_value.isInt()) {
    LOG_ERROR("Is_null field is not an integer. json value=%s", visible_value.toStyledString().c_str());
    return RC::GENERIC_ERROR;
  }
  ...
  int is_null = isnull_value.asInt();
